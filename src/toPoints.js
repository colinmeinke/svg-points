            ...
            for (let j = 0; j < iterations; j++) {
                switch (upperCaseCommand) {
                    case 'M':
                        ...
                        if (commandParams.length > 0) { // for some cases, where commands/params comes `M20 20 50 40` and this should fix this issue
                            x = (relative && x ? x : 0) + commandParams.shift();
                            y = (relative && y ? y : 0) + commandParams.shift();
...
const getPointsFromPoints = ({
    closed,
    points
}) => {
    const numbers = points.split(/[\s,]+/)
        .map(n => parseFloat(n))
...
const getPointsFromRectWithCornerRadius = ({
    height,
    rx,
    ry,
    width,
...
const getPointsFromG = ({
    shapes
}) => shapes.map(s => toPoints(s))
